...
def init_word():
    ...
    for i in range(total):
        ...
        content = content[1:]
        vec.append([(float)(j) for j in content])
    f.close()
    word2id['UNK'] = len(word2id)
    word2id['BLANK'] = len(word2id)
    vec.append(np.random.normal(size = word_size, loc = 0, scale = 0.05))
    vec.append(np.zeros(word_size, dtype = np.float32))
...
def init_train_files(name):
    ...
    f = open(data_path + name + '.txt','r')
    total = (int)(f.readline().strip())
    sen_word = np.zeros((total, fixlen), dtype = np.int64)
    sen_pos1 = np.zeros((total, fixlen), dtype = np.int64)
    sen_pos2 = np.zeros((total, fixlen), dtype = np.int64)
    sen_mask = np.zeros((total, fixlen, 3), dtype = np.float32)
    sen_len = np.zeros((total), dtype = np.int64)
    sen_label = np.zeros((total), dtype = np.int64)
    instance_scope = []
    instance_triple = []
    instance_label = []
    for s in range(total):
        ...
        for i in range(fixlen):
            ...
            if i >= len(sentence):
                ...
                sen_mask[s][i] = [100, 0, 0]
            elif i - en_second<=0:
                sen_mask[s][i] = [0, 100, 0]
            else:
                sen_mask[s][i] = [0, 0, 100]
	    ...
	    instance_label.append(relation)
        instance_scope[len(instance_triple) - 1][1] = s
        if (s+1) % 100 == 0:
            print(s)
    return np.array(instance_triple), np.array(instance_scope), np.array(instance_label, dtype = np.int64), sen_len, sen_label, sen_word, sen_pos1, sen_pos2, sen_mask
...
instance_triple, instance_scope, instance_label, train_len, train_label, train_word, train_pos1, train_pos2, train_mask = init_train_files("train_sort")
np.save(export_path+'train_instance_triple', instance_triple)
np.save(export_path+'train_instance_scope', instance_scope)
np.save(export_path+'train_instance_label', instance_label)
